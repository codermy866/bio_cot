...
class MedicalViTImageEncoder(nn.Module):
    """
    医学专用 Vision Transformer 图像编码器
    使用医学预训练模型或增强的ViT架构
    支持单帧 [B, C, H, W] 或多帧 [B, T, C, H, W]
    - 输出统一为 [B, embed_dim]
    ...
    def __init__(
        ...
        if use_medical_pretrained:
            ...
            # 如果指定的模型不在列表中，使用标准ViT
            if model_name not in medical_model_names:
                model_name = 'vit_base_patch16_224'
        else:
            # 使用标准ViT
            model_name = model_name if model_name.startswith('vit_') else 'vit_base_patch16_224'
    ...
    def _encode_single(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if feat.dim() == 3:
            ...
            # 医学增强
            feat = self.medical_enhancement(feat)  # [B, C]
            ...
            feat = self.medical_enhancement(feat)
            if feat.size(1) != expected_in_dim:
                if feat.size(1) < expected_in_dim:
                    pad = expected_in_dim - feat.size(1)
                    feat = F.pad(feat, (0, pad))
