...
class ViTClinicalFusionModel(nn.Module):
    """ViT + Clinical Fusion Baseline模型"""
    ...
    def forward(self, f_oct, f_colpo, clinical_data):
        ...
        # 简单拼接融合（无OT）
        f_concat = torch.cat([f_image, f_clinical], dim=-1)  # [B, 1536]
...
def train_single_run(config, device, seed, run_id, output_dir):
    ...
    # 创建模型
    model = ViTClinicalFusionModel(embed_dim=768, num_classes=2).to(device)
...
def main():
    parser = argparse.ArgumentParser(description='训练ViT + Clinical Fusion Baseline')
    parser.add_argument('--experiment_name', type=str, default='baseline_vit_clinical_fusion',
                       help='实验名称')
