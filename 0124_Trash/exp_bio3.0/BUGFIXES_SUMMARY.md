...
### ✅ 漏洞1：ViT [CLS] Token的失效（已修复）

**问题**：
- 原实现使用ViT的[CLS] token作为特征，但[CLS]包含了全图信息（包括背景）
- Visual Note的过滤操作对[CLS]无效，导致过滤失效
...
**问题**：
- 使用.npy数组格式保存embeddings，依赖顺序对齐
- 如果CSV读取顺序不一致或shuffle，会导致图像和诊断笔记错配
...
**问题**：
- 如果`lambda_sparse`过大，模型可能让attention map全变0
- 导致梯度消失，模型不再看图
...
1. **兼容性**：Dataset仍然支持.npy格式（向后兼容），但推荐使用.pt格式
