...
# -*- coding: utf-8 -*-
"""
Baseline 2: Standard CLIP Baseline
标准CLIP方法 - 无Knowledge Notes，无Visual Notes，有对比学习
"""
...
class StandardCLIPModel(nn.Module):
    """Standard CLIP Baseline模型"""
    ...
    def forward(self, f_oct, f_colpo, clinical_data):
        ...
        Args:
            ...
            clinical_data: [B, 3] 临床数据
        """
        # 编码临床特征
        f_clinical = self.clinical_encoder(clinical_data)  # [B, 768]
        
        # 多模态融合（加权）
        f_image = 0.6 * f_oct + 0.4 * f_colpo  # [B, 768]
        
        # Cross-Attention融合
        f_image_expanded = f_image.unsqueeze(1)  # [B, 1, 768]
        f_clinical_expanded = f_clinical.unsqueeze(1)  # [B, 1, 768]
        
        attn_out, _ = self.cross_attn(
            query=f_image_expanded,
            key=f_clinical_expanded,
            value=f_clinical_expanded
        )  # [B, 1, 768]
        
        f_image_attn = f_image_expanded + attn_out  # [B, 1, 768]
        f_image_attn = f_image_attn.squeeze(1)  # [B, 768]
        
        # 拼接融合
        f_concat = torch.cat([f_image_attn, f_clinical], dim=-1)  # [B, 1536]
...
def train_single_run(config, device, seed, run_id, output_dir):
    ...
    # 创建模型
    model = StandardCLIPModel(embed_dim=768, num_classes=2).to(device)
...
def main():
    parser = argparse.ArgumentParser(description='训练Standard CLIP Baseline')
    parser.add_argument('--experiment_name', type=str, default='baseline_standard_clip',
                       help='实验名称')
