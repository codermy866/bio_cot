...
CONFIG_PATHS = {
    ...
    # 🔥 5.0特性的消融实验（新增）
    "w/o_hierarchical": "ablation_studies/w/o_hierarchical/config.py",
    "w/o_noise_aware": "ablation_studies/w/o_noise_aware/config.py",
    "w/o_clinical_evolver": "ablation_studies/w/o_clinical_evolver/config.py",
    "w/o_text_adapter": "ablation_studies/w/o_text_adapter/config.py",
}
...
def is_experiment_running(exp_name):
    ...
    pattern_abs = str(config_path.resolve())
    pattern_rel = CONFIG_PATHS[exp_name]
    ...
    try:
        result = subprocess.run(
            [
                "bash",
                "-lc",
                f"ps aux | grep 'train_bio_cot_v3.2.py' | grep -v grep | "
                ...
                with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    if epoch_matches:
                        ...
                        if current_epoch > last_epoch:
                            print(f"  📊 {exp_name}: Epoch {current_epoch}/20")
...
def main():
    print("=" * 80)
    print("🔄 整合5.0优势的消融实验自动运行脚本（带实时监控）")
    print("=" * 80)
    ...
    print(f"\n需要运行的实验 ({len(incomplete_experiments)}个):")
    ...
    for i, exp in enumerate(incomplete_experiments, 1):
        ...
        # 检查是否正在运行
        if is_experiment_running(exp):
            print(f"⏳ 实验正在运行中: {exp}")
            # 等待完成
            log_dir = ROOT / "ablation_studies" / exp / "logs"
            latest_log = max(log_dir.glob("nohup_*.log"), key=lambda p: p.stat().st_mtime, default=None)
            if latest_log:
                print(f"   日志: {latest_log}")
                # 从进程列表中查找PID
                try:
                    result = subprocess.run(
                        ["bash", "-lc", f"ps aux | grep 'train_bio_cot_v3.2.py' | grep '{exp}' | grep -v grep | awk '{{print $2}}'"],
                        capture_output=True,
                        text=True
                    )
                    if result.stdout.strip():
                        pid = int(result.stdout.strip().split('\n')[0])
                        wait_for_completion(exp, pid, latest_log)
                    else:
                        print("   无法找到进程ID，等待60秒后继续...")
                        time.sleep(60)
                except Exception as e:
                    print(f"   检查进程时出错: {e}")
                    time.sleep(60)
