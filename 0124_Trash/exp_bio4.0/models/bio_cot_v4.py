...
# -*- coding: utf-8 -*-
"""
Bio-COT 4.0: Language-Anchored Causal Transport (LACT框架)
Frozen VLM + Trainable Adapter 机制
核心改进：引入文本锚点对齐机制，实现视觉-文本语义对齐
"""
...
class BioCOT_V4(nn.Module):
    """
    Bio-COT 4.0模型（LACT框架）
    
    核心改进：
    1. 替换知识检索器为 VLMAugmentedRetriever（Frozen VLM + Trainable Adapter）
    2. 引入文本锚点对齐机制（z_anchor）
    3. 使用LACT Loss（Language-Anchored Causal Transport Loss）
    """
    ...
    def forward(
        ...
        clinical_data: Optional[List[str]] = None,  # 临床信息列表（可选）
        image_names: Optional[List[str]] = None,  # ⚠️ 新增：图像文件名列表（必需）
        ...
        Args:
            ...
            clinical_data: list of strings 临床信息（可选）
...
# 创建模型工厂函数
def create_bio_cot_v4(config) -> BioCOT_V4:
    """
    创建Bio-COT v4模型
    
    Args:
        config: 配置对象（BioCOT_v4_Config）
