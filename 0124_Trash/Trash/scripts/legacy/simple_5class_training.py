...
class Simple5ClassDataset(Dataset):
    """简化的5分类数据集"""
    ...
    def __getitem__(self, idx):
        # 创建虚拟数据用于快速测试
        oct_images = torch.randn(48, 3, 224, 224)  # OCT序列
...
def train_simple_5class():
    ...
    # 创建数据集
    train_dataset = Simple5ClassDataset('5centers_multi_5class/train/train_labels.csv', num_samples=50)
    test_dataset = Simple5ClassDataset('5centers_multi_5class/test/test_labels.csv', num_samples=20)
    ...
    # 保存结果
    os.makedirs('simple_5class_results', exist_ok=True)
    ...
    }, 'simple_5class_results/simple_model_5class.pth')
    
    # 保存训练历史
    with open('simple_5class_results/training_history.json', 'w') as f:
    ...
    plt.tight_layout()
    plt.savefig('simple_5class_results/training_curves.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    print(f"\n🎉 简化5分类训练完成！")
    print(f"📁 结果保存在: simple_5class_results/")
