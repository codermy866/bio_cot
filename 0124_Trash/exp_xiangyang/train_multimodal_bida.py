...
def extract_features(images, resnet, device):
    ...
    if images.dim() == 4:  # [B, C, H, W]
        ...
        features = features_flat.view(B, F, -1).mean(dim=1)  # [B, 2048]
        return features
    else:
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, args):
    ...
    for batch in pbar:
        ...
        orth_loss = outputs.get('orthogonal_loss', torch.tensor(0.0, device=device))
        adv_loss = outputs.get('adversarial_loss', torch.tensor(0.0, device=device))
        
        total_loss = cls_loss + args.lambda_kl * kl_loss + args.lambda_orth * orth_loss + args.lambda_adv * adv_loss
        ...
        pbar.set_postfix({'loss': f'{total_loss.item():.4f}', 'acc': f'{acc:.4f}'})
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_metrics['acc'] > best_val_acc:
            ...
            }, args.checkpoint_dir / f'best_model_bida_{timestamp}.pth')
            print(f"  ✅ 保存最佳模型 (Val Acc: {best_val_acc:.4f}, Val AUC: {best_val_auc:.4f})")
    
    # 最终评估
    print("📊 使用最佳模型进行最终评估...")
    checkpoint = torch.load(args.checkpoint_dir / f'best_model_bida_{timestamp}.pth', map_location=device)
    ...
    results_file = args.output_dir / f'results_bida_{timestamp}.json'
    with open(results_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"✅ 结果已保存: {results_file}")
    
    # 可视化
    plot_training_curves(history, args.output_dir / f'training_curves_bida_{timestamp}.png')
    plot_confusion_matrix(
        final_metrics['labels'],
        final_metrics['preds'],
        args.output_dir / f'confusion_matrix_bida_{timestamp}.png'
    )
