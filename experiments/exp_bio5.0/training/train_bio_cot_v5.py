...
from models.bio_cot_v5 import BioCOT_V5, create_bio_cot_v5
from data.dataset_v5 import FiveCentersMultimodalDatasetV5
from config import BioCOT_v5_Config
try:
...
def visualize_training(history: Dict, log_dir: Path, timestamp: str, best_auc: float):
    ...
    fig.suptitle(f'Bio-COT 5.0 (mHC) Training Results (Best AUC: {best_auc:.4f})', 
...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, config, log_print=None):
    ...
    log_print(f"\n{'='*80}")
    log_print(f"📊 Epoch {epoch}/{config.num_epochs} - 训练阶段 (Bio-COT 5.0 mHC)")
    log_print(f"{'='*80}")
    ...
    for batch_idx, batch in enumerate(pbar):
        # ⚠️ 关键改动：获取图像文件名和临床特征（用于VLM检索和mHC融合）
        image_names = batch['image_name']  # List[str] 长度=B
        clinical_info = batch.get('clinical_info_str', None)  # List[str] 或None
        clinical_features = batch['clinical_features'].to(device, non_blocking=True)  # 🔥 [B, 7] Tensor用于mHC
        ...
        # 🔥🔥🔥 Bio-COT 5.0 核心改动：前向传播（传入clinical_features Tensor用于mHC融合）🔥🔥🔥
        outputs = model(
            images=combined_features,  # [B, N, D] Patch特征
            clinical_features=clinical_features,  # 🔥 [B, 7] Tensor用于mHC融合（Stage 1）
            clinical_data=clinical_info,  # List[str] 或None（用于VLM）
            image_names=image_names,  # List[str] 必需（用于VLM检索）
...
def validate(model, dataloader, criterion, device, epoch, config, log_print=None):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # 前向传播（传入clinical_features用于mHC融合）
            outputs = model(
                images=combined_features,
                clinical_features=clinical_features,  # 🔥 [B, 7] Tensor用于mHC融合
...
def main():
    ...
    # 🔥 新增：支持消融实验的命令行参数
    import argparse
    parser = argparse.ArgumentParser(description="Bio-COT 5.0 (mHC框架) 训练脚本")
    ...
    if args.ablation == 'baseline':
        ...
        print("=" * 80)
        print("🚀 Bio-COT 5.0 (Full Model with mHC) - 完整模型")
        print("=" * 80)
    ...
    log_file = Path(config.log_dir) / f"train_bio_cot_v5_{timestamp}.log"
    ...
    log_print("=" * 80)
    log_print("Bio-COT 5.0 (mHC框架): Manifold-Constrained Hyper-Connections")
    log_print("=" * 80)
    ...
    # 创建模型
    log_print("\n📊 正在创建Bio-COT 5.0模型 (mHC框架)...")
    model = create_bio_cot_v5(config)
