...
from models.MedicalViT.medical_vit_image_encoder import MedicalViTImageEncoder
from models.cnn_multimodal_model import ClinicalEncoder, CrossModalFusion


class MedicalViTMultimodalTransformer(nn.Module):
    """
    多模态（OCT + Colposcopy + Clinical）分类模型，图像编码器使用医学专用 ViT
    ...
    def __init__(
        ...
        use_medical_pretrained: bool = True,
    ):
        super().__init__()

        self.oct_encoder = MedicalViTImageEncoder(
            embed_dim=embed_dim,
            dropout=dropout,
            ...
            use_medical_pretrained=use_medical_pretrained,
        )

        self.col_encoder = MedicalViTImageEncoder(
            embed_dim=embed_dim,
            dropout=dropout,
...
class MedicalViTOctClassifier(nn.Module):
    """
    OCT 单模态分类模型（医学ViT 图像编码 + 帧间注意力聚合 + 分类头）
