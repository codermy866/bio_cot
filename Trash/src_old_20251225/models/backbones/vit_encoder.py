...
from models.ViT.vit_image_encoder import ViTImageEncoder
from models.cnn_multimodal_model import ClinicalEncoder, CrossModalFusion


class ViTMultimodalTransformer(nn.Module):
    """
    多模态（OCT + Colposcopy + Clinical）分类模型，图像编码器使用传统 ViT
    ...
    def __init__(
        ...
        self.oct_encoder = ViTImageEncoder(
            embed_dim=embed_dim,
            dropout=dropout,
        ...
        self.col_encoder = ViTImageEncoder(
            embed_dim=embed_dim,
            dropout=dropout,
...
class ViTOctClassifier(nn.Module):
    """
    OCT 单模态分类模型（ViT 图像编码 + 帧间注意力聚合 + 分类头）
