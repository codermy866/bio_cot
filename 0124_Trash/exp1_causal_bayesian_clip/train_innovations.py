#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
CLIP训练脚本 - 逐步加入创新点
阶段1: 创新点1（贝叶斯不确定性量化）
阶段2: 创新点1 + 创新点2（自适应因果图学习）
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
from torch.utils.data import DataLoader
from torch.cuda.amp import autocast, GradScaler
import numpy as np
from sklearn.metrics import (
    accuracy_score, f1_score, roc_auc_score, precision_score, 
    recall_score, confusion_matrix, roc_curve
)
import time
from datetime import datetime
import os
import json
import sys
from tqdm import tqdm
from pathlib import Path

# 添加项目根目录到路径
project_root = Path(__file__).resolve().parents[1]
if str(project_root) not in sys.path:
    sys.path.insert(0, str(project_root))

# 导入模型
try:
    from src.models.adaptive_causal_intervention_clip import AdaptiveCausalInterventionCLIP
except ImportError:
    sys.path.insert(0, str(project_root / 'src'))
    from models.adaptive_causal_intervention_clip import AdaptiveCausalInterventionCLIP

from utils.enhanced_multimodal_dataset import EnhancedMultimodalCervicalDataset
from utils.advanced_clinical_metrics import calculate_clinical_metrics

# 导入特征提取器
from models.SwinT.swin_image_encoder import SwinTImageEncoder


class FeatureExtractor(nn.Module):
    """特征提取器（使用Swin-T backbone）"""
    def __init__(self, embed_dim=768, device='cuda', fine_tune_last_n_layers=999):
        super().__init__()
        self.device = device
        self.embed_dim = embed_dim
        
        # OCT编码器
        self.oct_encoder = SwinTImageEncoder(
            embed_dim=embed_dim,
            num_frames=48,
            model_name='swin_tiny_patch4_window7_224',
            pretrained=True,
            input_size=192,
            use_frame_attention=False
        ).to(device)
        
        # Colposcopy编码器
        self.col_encoder = SwinTImageEncoder(
            embed_dim=embed_dim,
            num_frames=3,
            model_name='swin_tiny_patch4_window7_224',
            pretrained=True,
            input_size=192,
            use_frame_attention=False
        ).to(device)
        
        # 完全解冻（参考成功模型）
        for param in self.oct_encoder.parameters():
            param.requires_grad = True
        for param in self.col_encoder.parameters():
            param.requires_grad = True
    
    def forward(self, oct_images, col_images):
        oct_feat = self.oct_encoder(oct_images)
        col_feat = self.col_encoder(col_images)
        return oct_feat, col_feat


def prepare_loaders(data_path='5centers_multi', batch_size=5, input_size=192, oct_frames=48, use_strong_aug=True):
    """准备数据加载器"""
    print("📥 加载数据集...")
    
    class Args:
        def __init__(self, data_path):
            self.data_path = data_path
            self.input_size = input_size
            self.oct_num_frames = oct_frames
            self.col_num_frames = 3
            self.oct_cache_dir = 'oct_cache_optimized'
            self.use_text_contrastive = False
            self.oct_points = 12
            self.oct_frames_per_point = 10
            self.use_pretrained_backbones = True
    
    args = Args(data_path)
    
    # 数据增强
    if use_strong_aug:
        try:
            from timm.data import create_transform
            train_transform = create_transform(
                input_size=input_size,
                is_training=True,
                auto_augment='rand-m9-mstd0.5-inc1',
                interpolation='bicubic',
                re_prob=0.25,
                re_mode='pixel',
                re_count=1,
            )
            val_transform = create_transform(
                input_size=input_size,
                is_training=False,
                interpolation='bicubic',
            )
        except Exception:
            from torchvision import transforms
            train_transform = transforms.Compose([
                transforms.Resize((input_size, input_size)),
                transforms.RandomHorizontalFlip(p=0.5),
                transforms.RandomRotation(15),
                transforms.ColorJitter(brightness=0.2, contrast=0.2),
                transforms.ToTensor(),
                transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
            val_transform = transforms.Compose([
                transforms.Resize((input_size, input_size)),
                transforms.ToTensor(),
                transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            ])
    else:
        train_transform = None
        val_transform = None
    
    # 训练集
    train_dataset = EnhancedMultimodalCervicalDataset(
        root=os.path.join(data_path, 'train'),
        is_train='train',
        args=args,
        transform=train_transform,
        use_enhanced_oct=True,
        cache_oct_features=True
    )
    
    # 验证集
    val_dataset = EnhancedMultimodalCervicalDataset(
        root=os.path.join(data_path, 'test'),
        is_train='test',
        args=args,
        transform=val_transform,
        use_enhanced_oct=True,
        cache_oct_features=True
    )
    
    def collate_fn(batch):
        """自定义collate函数"""
        oct_images = []
        col_images = []
        clinical_features = []
        labels = []
        
        for item in batch:
            if isinstance(item, dict):
                oct_img = item.get('oct_images', item.get('oct_features'))
                col_img = item.get('col_images', item.get('colposcopy_features'))
                clinical_feat = item.get('clinical_features')
                label = item.get('label', item.get('labels'))
            elif isinstance(item, (list, tuple)) and len(item) >= 4:
                oct_img, col_img, clinical_feat, label = item[:4]
            else:
                continue
            
            if not isinstance(oct_img, torch.Tensor):
                oct_img = torch.tensor(oct_img) if oct_img is not None else torch.zeros(1)
            if not isinstance(col_img, torch.Tensor):
                col_img = torch.tensor(col_img) if col_img is not None else torch.zeros(1)
            if not isinstance(clinical_feat, torch.Tensor):
                clinical_feat = torch.tensor(clinical_feat) if clinical_feat is not None else torch.zeros(7)
            if not isinstance(label, torch.Tensor):
                label = torch.tensor(label, dtype=torch.long) if label is not None else torch.tensor(0, dtype=torch.long)
            
            if oct_img.dim() == 4:
                oct_img = oct_img.unsqueeze(0)
            if col_img.dim() == 4:
                col_img = col_img.unsqueeze(0)
            if clinical_feat.dim() == 1:
                clinical_feat = clinical_feat.unsqueeze(0)
            if label.dim() == 0:
                label = label.unsqueeze(0)
            
            oct_images.append(oct_img)
            col_images.append(col_img)
            clinical_features.append(clinical_feat)
            labels.append(label)
        
        if len(oct_images) == 0:
            raise ValueError("Batch is empty")
        
        try:
            if oct_images[0].dim() == 5:
                oct_images = torch.cat(oct_images, dim=0)
            else:
                oct_images = torch.stack(oct_images)
        except:
            oct_images = torch.stack([img.squeeze(0) if img.dim() > 4 else img for img in oct_images])
        
        try:
            if col_images[0].dim() == 5:
                col_images = torch.cat(col_images, dim=0)
            else:
                col_images = torch.stack(col_images)
        except:
            col_images = torch.stack([img.squeeze(0) if img.dim() > 4 else img for img in col_images])
        
        clinical_features = torch.cat(clinical_features, dim=0) if clinical_features[0].dim() == 2 else torch.stack(clinical_features)
        labels = torch.cat(labels, dim=0) if labels[0].dim() == 1 else torch.stack(labels)
        labels = labels.squeeze() if labels.dim() > 1 else labels
        
        return {
            'oct_images': oct_images,
            'col_images': col_images,
            'clinical_features': clinical_features,
            'labels': labels
        }
    
    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size,
        shuffle=True,
        num_workers=8,
        prefetch_factor=4,
        persistent_workers=True,
        pin_memory=True,
        collate_fn=collate_fn
    )
    
    val_loader = DataLoader(
        val_dataset,
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        pin_memory=True,
        collate_fn=collate_fn
    )
    
    print(f"✅ 训练集: {len(train_dataset)} 样本")
    print(f"✅ 验证集: {len(val_dataset)} 样本")
    
    return train_loader, val_loader


class FocalLoss(nn.Module):
    """Focal Loss with Label Smoothing"""
    def __init__(self, alpha=None, gamma=3.0, label_smoothing=0.1):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.label_smoothing = label_smoothing
    
    def forward(self, inputs, targets):
        num_classes = inputs.size(1)
        if self.label_smoothing > 0:
            confidence = 1.0 - self.label_smoothing
            log_probs = nn.functional.log_softmax(inputs, dim=1)
            with torch.no_grad():
                true_dist = torch.zeros_like(log_probs)
                true_dist.fill_(self.label_smoothing / (num_classes - 1))
                true_dist.scatter_(1, targets.data.unsqueeze(1), confidence)
            ce_loss = -torch.sum(true_dist * log_probs, dim=1)
        else:
            ce_loss = nn.CrossEntropyLoss(reduction='none')(inputs, targets)
        
        pt = torch.exp(-ce_loss)
        focal_loss = ((1 - pt) ** self.gamma) * ce_loss
        
        if self.alpha is not None:
            alpha_t = self.alpha[targets]
            focal_loss = alpha_t * focal_loss
        
        return focal_loss.mean()


def train_one_epoch(model, feature_extractor, train_loader, criterion, optimizer, scaler, device, epoch, num_epochs, 
                    kl_weight=0.01, contrastive_weight=0.1, intervention_weight=0.05,
                    current_best_auc=0.0, enable_auxiliary_after_auc=0.5):
    """
    训练一个epoch
    
    Args:
        current_best_auc: 当前最佳AUC，用于决定是否启用辅助损失
        enable_auxiliary_after_auc: 当AUC达到此值后启用辅助损失
    """
    # 根据当前AUC动态调整损失权重
    if current_best_auc < enable_auxiliary_after_auc:
        # 如果AUC还没达到阈值，暂时关闭所有辅助损失
        effective_kl_weight = 0.0
        effective_contrastive_weight = 0.0
        effective_intervention_weight = 0.0
    else:
        # AUC达到阈值后，逐步启用辅助损失
        effective_kl_weight = kl_weight
        effective_contrastive_weight = contrastive_weight
        effective_intervention_weight = intervention_weight
    
    model.train()
    feature_extractor.train()
    total_loss = 0
    total_kl_loss = 0
    total_contrastive_loss = 0
    total_intervention_loss = 0
    all_preds = []
    all_labels = []
    
    pbar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs}')
    
    for batch_idx, batch in enumerate(pbar):
        if isinstance(batch, dict):
            oct_images = batch.get('oct_images')
            col_images = batch.get('col_images')
            clinical_features = batch.get('clinical_features')
            labels = batch.get('labels')
        else:
            if len(batch) >= 4:
                oct_images, col_images, clinical_features, labels = batch[:4]
            else:
                continue
        
        oct_images = oct_images.to(device)
        col_images = col_images.to(device)
        clinical_features = clinical_features.to(device)
        labels = labels.to(device)
        
        optimizer.zero_grad()
        
        with autocast():
            # 特征提取
            oct_feat, col_feat = feature_extractor(oct_images, col_images)
            
            # CLIP模型前向传播
            output = model(
                oct_feat=oct_feat,
                colpo_feat=col_feat,
                clinical_feat=clinical_features,
                labels=labels,
                return_uncertainty=True,
                return_intervention=True,
                return_causal_graph=True
            )
            
            logits = output['logits']
            
            # 分类损失
            cls_loss = criterion(logits, labels)
            
            # KL损失（创新点1：贝叶斯不确定性量化）
            # 优化：使用更合理的归一化，按特征维度归一化
            kl_loss = torch.tensor(0.0, device=device)
            if effective_kl_weight > 0:
                means = []
                vars_list = []
                if 'oct_mean' in output:
                    means.append(output['oct_mean'])
                    vars_list.append(output['oct_var'])
                if 'colpo_mean' in output:
                    means.append(output['colpo_mean'])
                    vars_list.append(output['colpo_var'])
                if 'clinical_mean' in output:
                    means.append(output['clinical_mean'])
                    vars_list.append(output['clinical_var'])
                
                if len(means) > 0:
                    for mean, var in zip(means, vars_list):
                        var_clamped = torch.clamp(var, min=1e-6, max=1.0)  # 限制方差范围
                        mean_clamped = torch.clamp(mean, min=-5.0, max=5.0)  # 限制均值范围
                        # 按特征维度归一化（更稳定）
                        kl_per_feature = 0.5 * (mean_clamped.pow(2) + var_clamped - var_clamped.log() - 1)
                        kl_loss = kl_loss + kl_per_feature.mean()  # 先平均特征维度，再平均样本
                    kl_loss = kl_loss / len(means)  # 平均所有模态
                    # 进一步限制KL损失范围（防止过大）
                    kl_loss = torch.clamp(kl_loss, min=0.0, max=5.0)  # 大幅降低上限
            
            # 对比学习损失（创新点4：分层对比学习）
            contrastive_loss = torch.tensor(0.0, device=device)
            if effective_contrastive_weight > 0:
                if 'contrastive_loss' in output:
                    contrastive_loss = output['contrastive_loss']
                elif 'contrastive' in output and isinstance(output['contrastive'], dict):
                    contrastive_loss = output['contrastive'].get('total_contrastive_loss', torch.tensor(0.0, device=device))
                # 限制对比损失范围，防止NaN
                if isinstance(contrastive_loss, torch.Tensor):
                    contrastive_loss = torch.clamp(contrastive_loss, min=0.0, max=100.0)
                    if torch.isnan(contrastive_loss) or torch.isinf(contrastive_loss):
                        contrastive_loss = torch.tensor(0.0, device=device)
            
            # 因果干预损失（创新点3：因果干预）
            intervention_loss = torch.tensor(0.0, device=device)
            if effective_intervention_weight > 0:
                if 'intervention_loss' in output:
                    intervention_loss = output['intervention_loss']
                elif 'intervention' in output and isinstance(output['intervention'], dict):
                    # 从intervention字典中提取损失，如果没有则使用干预强度作为损失
                    intervention_loss = output['intervention'].get('intervention_loss', 
                                                                  output['intervention'].get('intervention_strength', 
                                                                                            torch.tensor(0.0, device=device)))
            
            # 总损失（使用有效的权重）
            loss = cls_loss + effective_kl_weight * kl_loss + effective_contrastive_weight * contrastive_loss + effective_intervention_weight * intervention_loss
            
            # NaN检测和处理
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"⚠️  检测到NaN/Inf损失，跳过此batch")
                print(f"   Cls: {cls_loss.item():.4f}, KL: {kl_loss.item() if isinstance(kl_loss, torch.Tensor) else 0.0:.4f}, "
                      f"CLIP: {contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else 0.0:.4f}, "
                      f"Interv: {intervention_loss.item() if isinstance(intervention_loss, torch.Tensor) else 0.0:.4f}")
                # 只使用分类损失
                loss = cls_loss
        
        scaler.scale(loss).backward()
        scaler.unscale_(optimizer)
        torch.nn.utils.clip_grad_norm_(list(model.parameters()) + list(feature_extractor.parameters()), max_norm=1.0)
        scaler.step(optimizer)
        scaler.update()
        
        total_loss += loss.item()
        total_kl_loss += kl_loss.item() if isinstance(kl_loss, torch.Tensor) else 0.0
        total_contrastive_loss += contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else 0.0
        total_intervention_loss += intervention_loss.item() if isinstance(intervention_loss, torch.Tensor) else 0.0
        preds = logits.argmax(dim=1)
        all_preds.extend(preds.detach().cpu().numpy())
        all_labels.extend(labels.detach().cpu().numpy())
        
        # 显示有效的权重状态
        aux_status = "ON" if current_best_auc >= enable_auxiliary_after_auc else "OFF"
        # 计算当前准确率
        if len(all_labels) > 0:
            current_acc = accuracy_score(all_labels, all_preds)
        else:
            current_acc = 0.0
        pbar.set_postfix({
            'Loss': f'{loss.item():.4f}',
            'Cls': f'{cls_loss.item():.4f}',
            'KL': f'{kl_loss.item() if isinstance(kl_loss, torch.Tensor) else 0.0:.4f}',
            'CLIP': f'{contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else 0.0:.4f}',
            'Interv': f'{intervention_loss.item() if isinstance(intervention_loss, torch.Tensor) else 0.0:.4f}',
            'Acc': f'{current_acc:.2%}',
            'Aux': aux_status
        })
    
    avg_loss = total_loss / len(train_loader)
    avg_kl_loss = total_kl_loss / len(train_loader)
    avg_contrastive_loss = total_contrastive_loss / len(train_loader)
    avg_intervention_loss = total_intervention_loss / len(train_loader)
    acc = accuracy_score(all_labels, all_preds)
    
    return avg_loss, avg_kl_loss, avg_contrastive_loss, avg_intervention_loss, acc


def validate(model, feature_extractor, val_loader, criterion, device, kl_weight=0.01, contrastive_weight=0.1, intervention_weight=0.05):
    """验证"""
    model.eval()
    feature_extractor.eval()
    total_loss = 0
    total_kl_loss = 0
    all_preds = []
    all_labels = []
    all_probs = []
    
    with torch.no_grad():
        for batch in tqdm(val_loader, desc='验证中'):
            if isinstance(batch, dict):
                oct_images = batch.get('oct_images')
                col_images = batch.get('col_images')
                clinical_features = batch.get('clinical_features')
                labels = batch.get('labels')
            else:
                if len(batch) >= 4:
                    oct_images, col_images, clinical_features, labels = batch[:4]
                else:
                    continue
            
            oct_images = oct_images.to(device)
            col_images = col_images.to(device)
            clinical_features = clinical_features.to(device)
            labels = labels.to(device)
            
            with autocast():
                # 特征提取
                oct_feat, col_feat = feature_extractor(oct_images, col_images)
                
                # CLIP模型前向传播
                output = model(
                    oct_feat=oct_feat,
                    colpo_feat=col_feat,
                    clinical_feat=clinical_features,
                    return_uncertainty=True,
                    return_intervention=True,
                    return_causal_graph=True
                )
                
                logits = output['logits']
                
                # 分类损失
                cls_loss = criterion(logits, labels)
                
                # KL损失
                kl_loss = torch.tensor(0.0, device=device)
                if kl_weight > 0:
                    means = []
                    vars_list = []
                    if 'oct_mean' in output:
                        means.append(output['oct_mean'])
                        vars_list.append(output['oct_var'])
                    if 'colpo_mean' in output:
                        means.append(output['colpo_mean'])
                        vars_list.append(output['colpo_var'])
                    if 'clinical_mean' in output:
                        means.append(output['clinical_mean'])
                        vars_list.append(output['clinical_var'])
                    
                    if len(means) > 0:
                        for mean, var in zip(means, vars_list):
                            var_clamped = torch.clamp(var, min=1e-6, max=10.0)
                            mean_clamped = torch.clamp(mean, min=-10.0, max=10.0)
                            kl_per_sample = 0.5 * (mean_clamped.pow(2) + var_clamped - var_clamped.log() - 1).sum(dim=-1)
                            kl_loss = kl_loss + kl_per_sample.mean()
                        kl_loss = kl_loss / len(means)
                        kl_loss = torch.clamp(kl_loss, min=0.0, max=100.0)
                
                # 对比学习损失
                contrastive_loss = torch.tensor(0.0, device=device)
                if contrastive_weight > 0:
                    if 'contrastive_loss' in output:
                        contrastive_loss = output['contrastive_loss']
                    elif 'contrastive' in output and isinstance(output['contrastive'], dict):
                        contrastive_loss = output['contrastive'].get('total_contrastive_loss', torch.tensor(0.0, device=device))
                    if isinstance(contrastive_loss, torch.Tensor):
                        contrastive_loss = torch.clamp(contrastive_loss, min=0.0, max=100.0)
                        if torch.isnan(contrastive_loss) or torch.isinf(contrastive_loss):
                            contrastive_loss = torch.tensor(0.0, device=device)
                
                # 因果干预损失
                intervention_loss = torch.tensor(0.0, device=device)
                if intervention_weight > 0:
                    if 'intervention_loss' in output:
                        intervention_loss = output['intervention_loss']
                    elif 'intervention' in output and isinstance(output['intervention'], dict):
                        intervention_loss = output['intervention'].get('intervention_loss', 
                                                                      output['intervention'].get('intervention_strength', 
                                                                                                torch.tensor(0.0, device=device)))
                
                loss = cls_loss + kl_weight * kl_loss + contrastive_weight * contrastive_loss + intervention_weight * intervention_loss
            
            total_loss += loss.item()
            total_kl_loss += kl_loss.item() if isinstance(kl_loss, torch.Tensor) else 0.0
            probs = torch.softmax(logits, dim=1)
            preds = logits.argmax(dim=1)
            
            all_preds.extend(preds.detach().cpu().numpy().tolist())
            all_labels.extend(labels.detach().cpu().numpy().tolist())
            all_probs.extend(probs[:, 1].detach().cpu().numpy().tolist())
    
    avg_loss = total_loss / len(val_loader)
    avg_kl_loss = total_kl_loss / len(val_loader)
    acc = accuracy_score(all_labels, all_preds)
    
    # 计算AUC
    auc = 0.5
    if len(all_labels) > 0 and len(set(all_labels)) > 1:
        probs_np = np.asarray(all_probs, dtype=float)
        if np.std(probs_np) > 1e-8:
            try:
                auc = roc_auc_score(all_labels, probs_np)
            except:
                auc = 0.5
    
    # 计算最优阈值
    optimal_threshold = 0.5
    optimal_acc = acc
    optimal_f1 = f1_score(all_labels, all_preds, zero_division=0)
    optimal_sensitivity = recall_score(all_labels, all_preds, zero_division=0)
    optimal_specificity = 0.0
    
    if len(all_labels) > 0 and len(set(all_labels)) > 1:
        probs_np = np.asarray(all_probs, dtype=float)
        if np.std(probs_np) > 1e-8:
            fpr, tpr, thresholds = roc_curve(all_labels, probs_np)
            youden_index = tpr - fpr
            optimal_idx = int(np.argmax(youden_index))
            optimal_threshold = float(thresholds[optimal_idx])
        
        opt_preds = (probs_np >= optimal_threshold).astype(int)
        optimal_acc = accuracy_score(all_labels, opt_preds)
        optimal_f1 = f1_score(all_labels, opt_preds, zero_division=0)
        optimal_sensitivity = recall_score(all_labels, opt_preds, zero_division=0)
        tn, fp, fn, tp = confusion_matrix(all_labels, opt_preds, labels=[0,1]).ravel()
        denom = (tn + fp)
        optimal_specificity = float(tn / denom) if denom > 0 else 0.0
    
    try:
        clinical_metrics = calculate_clinical_metrics(all_labels, all_preds, all_probs)
    except:
        clinical_metrics = {}
    
    return {
        'loss': avg_loss,
        'kl_loss': avg_kl_loss,
        'accuracy': acc,
        'auc': float(auc),
        'f1': optimal_f1,
        'precision': precision_score(all_labels, all_preds, zero_division=0),
        'recall': optimal_sensitivity,
        'optimal_threshold': float(optimal_threshold),
        'optimal_accuracy': optimal_acc,
        'optimal_f1': optimal_f1,
        'optimal_sensitivity': optimal_sensitivity,
        'optimal_specificity': optimal_specificity,
        'clinical_metrics': clinical_metrics
    }


def main():
    """主训练函数"""
    import argparse
    
    parser = argparse.ArgumentParser(description='CLIP训练 - 逐步加入创新点')
    parser.add_argument('--data_path', type=str, default='5centers_multi', help='数据路径')
    parser.add_argument('--output_dir', type=str, default='adaptive_causal_intervention_results', help='输出目录')
    parser.add_argument('--batch_size', type=int, default=5, help='批次大小')
    parser.add_argument('--num_epochs', type=int, default=30, help='训练轮数')
    parser.add_argument('--learning_rate', type=float, default=2.1e-5, help='学习率')
    parser.add_argument('--input_size', type=int, default=192, help='输入尺寸')
    parser.add_argument('--oct_frames', type=int, default=48, help='OCT帧数')
    parser.add_argument('--device', type=str, default='cuda', help='设备')
    parser.add_argument('--stage', type=int, default=4, help='阶段：1=创新点1, 2=创新点1+2, 4=所有创新点')
    parser.add_argument('--kl_weight', type=float, default=0.0, help='KL损失权重（暂时关闭，待AUC>0.5后启用）')
    parser.add_argument('--contrastive_weight', type=float, default=0.0, help='对比学习损失权重（暂时关闭，待AUC>0.5后启用）')
    parser.add_argument('--intervention_weight', type=float, default=0.0, help='因果干预损失权重（暂时关闭，待AUC>0.5后启用）')
    parser.add_argument('--enable_auxiliary_after_auc', type=float, default=0.5, help='当AUC达到此值后启用辅助损失')
    
    args = parser.parse_args()
    
    data_path = args.data_path
    output_dir = args.output_dir
    batch_size = args.batch_size
    num_epochs = args.num_epochs
    learning_rate = args.learning_rate
    input_size = args.input_size
    oct_frames = args.oct_frames
    device = args.device if torch.cuda.is_available() else 'cpu'
    stage = args.stage
    kl_weight = args.kl_weight
    contrastive_weight = args.contrastive_weight
    intervention_weight = args.intervention_weight
    enable_auxiliary_after_auc = args.enable_auxiliary_after_auc
    
    print("=" * 80)
    print("🚀 CLIP训练 - 逐步加入创新点")
    print("=" * 80)
    print(f"📁 数据路径: {data_path}")
    print(f"💻 使用设备: {device}")
    print(f"📦 批次大小: {batch_size}")
    print(f"🎯 训练轮数: {num_epochs}")
    print(f"📚 学习率: {learning_rate}")
    print(f"🖼️  输入尺寸: {input_size}")
    print(f"🎬 OCT帧数: {oct_frames}")
    
    if stage == 1:
        print(f"\n🎯 阶段1: 创新点1（贝叶斯不确定性量化）")
        print(f"   ✅ 启用: 贝叶斯编码器 + KL损失")
        print(f"   ❌ 关闭: 因果干预、自适应因果图、分层对比学习")
        use_causal_intervention = False
        use_adaptive_causal_graph = False
        use_hierarchical_contrastive = False
    elif stage == 2:
        print(f"\n🎯 阶段2: 创新点1 + 创新点2（自适应因果图学习）")
        print(f"   ✅ 启用: 贝叶斯编码器 + KL损失 + 自适应因果图")
        print(f"   ❌ 关闭: 因果干预、分层对比学习")
        use_causal_intervention = False
        use_adaptive_causal_graph = True
        use_hierarchical_contrastive = False
    elif stage == 4:
        print(f"\n🎯 阶段4: 所有创新点（完整版）")
        print(f"   ✅ 启用: 创新点1（贝叶斯不确定性量化）")
        print(f"   ✅ 启用: 创新点2（自适应因果图学习）")
        print(f"   ✅ 启用: 创新点3（因果干预）")
        print(f"   ✅ 启用: 创新点4（分层对比学习）")
        use_causal_intervention = True
        use_adaptive_causal_graph = True
        use_hierarchical_contrastive = True
    else:
        raise ValueError(f"Unknown stage: {stage}")
    
    print(f"   KL损失权重: {kl_weight}")
    print(f"   对比学习损失权重: {contrastive_weight}")
    print(f"   因果干预损失权重: {intervention_weight}")
    print("=" * 80)
    
    os.makedirs(output_dir, exist_ok=True)
    
    # 准备数据
    train_loader, val_loader = prepare_loaders(
        data_path, batch_size, input_size, oct_frames, use_strong_aug=True
    )
    
    # 创建特征提取器
    print("\n🔧 创建特征提取器...")
    feature_extractor = FeatureExtractor(embed_dim=768, device=device, fine_tune_last_n_layers=999)
    print(f"✅ 特征提取器创建成功（完全解冻）")
    
    # 创建CLIP模型
    print("\n🔧 创建自适应因果干预CLIP模型...")
    model = AdaptiveCausalInterventionCLIP(
        embed_dim=768,
        clinical_dim=7,
        num_classes=2,
        temperature=0.07,
        kl_weight=kl_weight,
        use_causal_intervention=use_causal_intervention,
        use_adaptive_causal_graph=use_adaptive_causal_graph,
        use_hierarchical_contrastive=use_hierarchical_contrastive,
        use_uncertainty_guided=False,  # 暂时关闭
        use_uncertainty_decomposition=True,  # 启用不确定性分解
        num_centers=0
    ).to(device)
    
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"✅ 模型创建成功")
    print(f"   总参数量: {total_params / 1e6:.2f}M")
    print(f"   可训练参数: {trainable_params / 1e6:.2f}M")
    
    # 计算类别权重
    class_alpha = torch.tensor([0.325, 0.675]).to(device)
    
    # 创建损失函数
    criterion = FocalLoss(
        alpha=class_alpha,
        gamma=3.0,
        label_smoothing=0.1
    )
    
    # 创建优化器
    all_params = list(model.parameters()) + list(feature_extractor.parameters())
    optimizer = optim.AdamW(
        all_params,
        lr=learning_rate,
        weight_decay=6e-4,
        betas=(0.9, 0.999)
    )
    
    # 学习率调度
    warmup_epochs = max(1, num_epochs // 10)
    def lr_lambda(epoch):
        if epoch < warmup_epochs:
            return (epoch + 1) / warmup_epochs
        else:
            progress = (epoch - warmup_epochs) / (num_epochs - warmup_epochs)
            return 0.5 * (1 + np.cos(np.pi * progress))
    
    scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda)
    
    scaler = GradScaler()
    
    # 训练历史
    history = {
        'train_loss': [],
        'train_kl_loss': [],
        'train_acc': [],
        'val_loss': [],
        'val_kl_loss': [],
        'val_acc': [],
        'val_auc': [],
        'val_f1': [],
        'best_auc': 0.0,
        'best_epoch': 0
    }
    
    print("\n🎓 开始训练...\n")
    best_auc = 0.0
    patience = 7
    patience_counter = 0
    min_delta = 0.0005
    
    for epoch in range(num_epochs):
        # 训练
        # 获取当前最佳AUC（处理空列表情况）
        val_auc_list = history.get('val_auc', [])
        current_best_auc = max(val_auc_list) if len(val_auc_list) > 0 else 0.0
        
        train_loss, train_kl_loss, train_contrastive_loss, train_intervention_loss, train_acc = train_one_epoch(
            model, feature_extractor, train_loader, criterion, optimizer, scaler, 
            device, epoch, num_epochs, kl_weight=kl_weight, 
            contrastive_weight=contrastive_weight, intervention_weight=intervention_weight,
            current_best_auc=current_best_auc, enable_auxiliary_after_auc=enable_auxiliary_after_auc
        )
        
        # 验证（验证时也使用动态权重）
        val_results = validate(model, feature_extractor, val_loader, criterion, device, 
                              kl_weight=kl_weight if current_best_auc >= enable_auxiliary_after_auc else 0.0,
                              contrastive_weight=contrastive_weight if current_best_auc >= enable_auxiliary_after_auc else 0.0,
                              intervention_weight=intervention_weight if current_best_auc >= enable_auxiliary_after_auc else 0.0)
        
        # 更新学习率
        scheduler.step()
        
        # 记录历史
        history['train_loss'].append(train_loss)
        history['train_kl_loss'].append(train_kl_loss)
        history['train_acc'].append(train_acc)
        history['val_loss'].append(val_results['loss'])
        history['val_kl_loss'].append(val_results['kl_loss'])
        history['val_acc'].append(val_results['accuracy'])
        history['val_auc'].append(val_results['auc'])
        history['val_f1'].append(val_results['f1'])
        
        # 打印结果
        print(f"\n{'='*80}")
        print(f"📊 Epoch [{epoch+1}/{num_epochs}] 结果")
        print(f"{'='*80}")
        print(f"训练集:")
        print(f"  Loss: {train_loss:.4f} (Cls: {train_loss-train_kl_loss*kl_weight-train_contrastive_loss*contrastive_weight-train_intervention_loss*intervention_weight:.4f}, KL: {train_kl_loss:.4f}, CLIP: {train_contrastive_loss:.4f}, Interv: {train_intervention_loss:.4f})")
        print(f"  Accuracy: {train_acc:.2%}")
        print(f"验证集:")
        print(f"  Loss: {val_results['loss']:.4f} (Cls: {val_results['loss']-val_results['kl_loss']*kl_weight:.4f}, KL: {val_results['kl_loss']:.4f})")
        print(f"  Accuracy: {val_results['accuracy']:.2%}")
        print(f"  AUC: {val_results['auc']:.4f}")
        print(f"  F1-Score: {val_results['f1']:.4f}")
        print(f"  最优阈值: {val_results['optimal_threshold']:.4f}")
        print(f"  最优准确率: {val_results['optimal_accuracy']:.2%}")
        print(f"  最优敏感性: {val_results['optimal_sensitivity']:.4f}")
        print(f"  最优特异性: {val_results['optimal_specificity']:.4f}")
        
        # Early Stopping
        if val_results['auc'] > best_auc + min_delta:
            best_auc = val_results['auc']
            patience_counter = 0
            history['best_auc'] = best_auc
            history['best_epoch'] = epoch + 1
            
            checkpoint = {
                'epoch': epoch + 1,
                'model_state_dict': model.state_dict(),
                'feature_extractor_state_dict': feature_extractor.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'scheduler_state_dict': scheduler.state_dict(),
                'best_auc': best_auc,
                'val_results': val_results,
                'history': history,
                'stage': stage
            }
            torch.save(checkpoint, os.path.join(output_dir, f'best_model_stage{stage}.pth'))
            print(f"\n💾 保存最佳模型 (Epoch {epoch+1}, AUC: {best_auc:.4f})")
        else:
            patience_counter += 1
        
        # Early Stopping
        if patience_counter >= patience:
            print(f"\n⏹️  Early Stopping触发 (patience={patience})")
            print(f"   最佳AUC: {best_auc:.4f} (Epoch {history['best_epoch']})")
            break
        
        # 保存训练历史
        def convert_to_serializable(obj):
            if isinstance(obj, (np.integer, np.int64)):
                return int(obj)
            elif isinstance(obj, (np.floating, np.float32, np.float64)):
                return float(obj)
            elif isinstance(obj, np.ndarray):
                return obj.tolist()
            elif isinstance(obj, torch.Tensor):
                return obj.detach().cpu().numpy().tolist()
            elif isinstance(obj, dict):
                return {k: convert_to_serializable(v) for k, v in obj.items()}
            elif isinstance(obj, list):
                return [convert_to_serializable(item) for item in obj]
            return obj
        
        with open(os.path.join(output_dir, f'training_history_stage{stage}.json'), 'w') as f:
            json.dump(convert_to_serializable(history), f, indent=2)
    
    print(f"\n✅ 训练完成！最佳AUC: {best_auc:.4f}")


if __name__ == '__main__':
    main()

