...
# 修复导入路径：使用相对导入
try:
    from .swin_image_encoder import SwinTImageEncoder
    from .cnn_encoder import ClinicalEncoder, CrossModalFusion
except ImportError:
    # 如果相对导入失败，尝试绝对导入
    try:
        from src.models.backbones.swin_image_encoder import SwinTImageEncoder
        from src.models.backbones.cnn_encoder import ClinicalEncoder, CrossModalFusion
    except ImportError:
        # 最后的回退方案（为了向后兼容）
