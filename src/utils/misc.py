...
class SmoothedValue(object):
    ...
    def __str__(self):
        return self.fmt.format(
            ...
            global_avg=self.global_avg,
            max=self.max,
            value=self.value) if self.fmt is not None else str(self.value)
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        if not isinstance(iterable, Iterable):
            raise RuntimeError(f"iterable must be an Iterable, got {type(iterable)}")
        total_time = 0.
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable):
                eta_seconds = total_time * (len(iterable) - i) / max(i, 1)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    memory = torch.cuda.max_memory_allocated() / MB
                else:
                ...
                if len(iterable) > 0:
                    print(log_msg.format(
                        i, len(iterable), eta=eta_string,
                        meters=str(self), time=total_time_str, memory=memory))
                else:
                    print(f"Warning: Data loader is empty. No iterations to log. {header}")
        total_time = time.time() - total_time_start
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        print(f'{header} Total time: {total_time_str} ({total_time / max(len(iterable), 1)} s / it)')
...
def init_distributed_mode(args):
    ...
    print('| distributed init (rank {}): {}'.format(
        args.rank, args), flush=True)
    dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                            world_size=args.world_size, rank=args.rank)
    dist.barrier()
    setup_for_distributed(args.rank == 0)

def save_model(args, model, model_without_ddp, optimizer, loss_scaler, epoch, mode='best'):
    output_dir = Path(args.output_dir) / args.task
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_paths = {
        'latest': output_dir / f'checkpoint_{mode}.pth'
    }
