...
# -*- coding: utf-8 -*-
"""
Mantel检验相关性分析 - 最终版（完全参考用户提供的图片样式）
- 左侧：下三角相关性矩阵（Pearson相关性数值 + Mantel检验方块大小和星号）
...
results_csv = OUTPUT_DIR / 'mantel_test_results_final.csv'
...
im = ax1.imshow(display_corr, cmap=cmap, aspect='auto', vmin=-1, vmax=1, 
...
for i in range(n_features):
    for j in range(i+1):
        ...
        # 方块颜色：根据Pearson相关性（使用colormap）
        norm_val = (pearson_r + 1) / 2  # 归一化到0-1
        box_color = cmap(norm_val)
        
        # 绘制方块（覆盖在热图上）
        rect = Rectangle((j - box_size/2, i - box_size/2),
                        box_size, box_size,
        ...
        # 添加Pearson相关性数值
        text_color = 'white' if abs(pearson_r) > 0.5 else 'black'
        ax1.text(j, i, f'{pearson_r:.2f}',
                ha='center', va='center',
                fontsize=9, fontweight='bold', color=text_color,
                transform=ax1.transData)
        
        # 添加显著性星号（根据Mantel's p值）
        if mantel_p < 0.001:
            sig = '***'
            sig_color = '#8B0000'
        elif mantel_p < 0.01:
...
ax1.set_xticklabels(feature_display_names, rotation=45, ha='right', fontsize=9)
ax1.set_yticklabels(feature_display_names, rotation=0, fontsize=9)

# 添加colorbar
cbar = plt.colorbar(im, ax=ax1, shrink=0.7, pad=0.02)
cbar.set_label("Pearson's correlation", fontsize=11, fontweight='bold')
...
# 特征节点：左侧垂直排列
feature_y_positions = np.linspace(1.5, -1.5, len(feature_display_names))
for i, node in enumerate(feature_display_names):
    pos[node] = (-2.0, feature_y_positions[i])

# 目标节点：右侧垂直排列
target_y_positions = np.linspace(0.8, -0.8, len(target_nodes))
for i, node in enumerate(target_nodes):
    pos[node] = (2.0, target_y_positions[i])
...
legend1 = ax2.legend(handles=legend_elements_p, loc='upper left',
                    ...
                    framealpha=0.95, edgecolor='black', frameon=True, fancybox=True)
legend1.get_frame().set_facecolor('white')
legend1.get_frame().set_linewidth(1.5)

# 图例2：Mantel's r value（线条粗细）- 左下角
legend_elements_r = [
    Line2D([0], [0], color='black', lw=5, label='r ≥ 0.4'),
    Line2D([0], [0], color='black', lw=3, label='0.2 ≤ r < 0.4'),
    Line2D([0], [0], color='black', lw=1.5, label='r < 0.2'),
]
...
# 保存
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'Mantel_Test_Final.png', dpi=300, bbox_inches='tight',
           facecolor=COLORS['background'])
plt.savefig(FIGURES_DIR / 'Mantel_Test_Final.pdf', dpi=300, bbox_inches='tight',
           facecolor=COLORS['background'])
plt.close()
print("✅ 最终版组合图已保存")
...
print()
print("生成的文件:")
print(f"  ✅ mantel_test_results_final.csv")
for fig_file in sorted(FIGURES_DIR.glob('Mantel_Test_Final.*')):
