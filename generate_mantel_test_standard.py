#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Mantel检验相关性分析 - 标准样式（完全参考用户提供的图片）
- 左侧：下三角相关性矩阵（Pearson相关性 + Mantel检验的方块大小和星号）
- 右侧：网络图（特征节点与目标节点的连接，颜色=p值，粗细=Mantel's r值）
"""

import os
import sys
from pathlib import Path
import warnings
warnings.filterwarnings('ignore')

import numpy as np
import pandas as pd
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from matplotlib.gridspec import GridSpec
from matplotlib.patches import Rectangle, FancyBboxPatch
import seaborn as sns
from scipy.spatial.distance import pdist, squareform
from scipy.stats import pearsonr
from sklearn.metrics import pairwise_distances
import networkx as nx

# 配置路径
ROOT = Path(__file__).parent
OUTPUT_DIR = ROOT / 'newlog_0126'
OUTPUT_DIR.mkdir(exist_ok=True)
FIGURES_DIR = OUTPUT_DIR / 'figures'
FIGURES_DIR.mkdir(exist_ok=True)

# 设置字体和样式
plt.rcParams['font.family'] = 'sans-serif'
plt.rcParams['font.sans-serif'] = ['DejaVu Sans', 'Arial', 'Helvetica']
plt.rcParams['font.size'] = 9
plt.rcParams['font.weight'] = 'bold'
plt.rcParams['figure.dpi'] = 300

# 统一配色：D69584到C7CCD6
COLORS = {
    'positive': '#D69584',
    'negative': '#C7CCD6',
    'center_0': '#D69584',
    'center_1': '#D2A392',
    'center_2': '#CEB1A0',
    'center_3': '#CABFAE',
    'center_4': '#C7CCD6',
    'background': '#FAFAFA',
}

print("=" * 80)
print("📊 Mantel检验相关性分析 - 标准样式")
print("=" * 80)

# ========================================
# 1. 加载数据
# ========================================
print("\n📊 步骤 1: 准备数据...")

data_file = ROOT / 'visualization' / 'data' / 'Complete_Dataset.csv'
if data_file.exists():
    print(f"📖 从文件加载数据: {data_file}")
    df = pd.read_csv(data_file)
    
    feature_cols = [f'feature_{i}' for i in range(10) if f'feature_{i}' in df.columns]
    if feature_cols:
        features = df[feature_cols].values
        labels = df['label'].values if 'label' in df.columns else None
        center_ids = df['center_id'].values if 'center_id' in df.columns else None
        
        # 特征名称映射
        feature_names = {
            'feature_0': 'OCT Texture',
            'feature_1': 'OCT Intensity',
            'feature_2': 'OCT Morphology',
            'feature_3': 'Colposcopy Texture',
            'feature_4': 'Colposcopy Color',
            'feature_5': 'Colposcopy Vascular',
            'feature_6': 'Clinical Age',
            'feature_7': 'Clinical HPV',
            'feature_8': 'Clinical TCT',
            'feature_9': 'Fused Feature',
        }
        
        feature_display_names = [feature_names.get(col, col) for col in feature_cols]
        print(f"✅ 加载了 {len(features)} 个样本，{len(feature_cols)} 个特征")
    else:
        features = None
        labels = None
        center_ids = None
        feature_display_names = None
else:
    features = None
    labels = None
    center_ids = None
    feature_display_names = None

if features is None:
    print("💡 生成模拟特征数据...")
    np.random.seed(42)
    n_samples = 200
    n_features = 10
    features = np.random.randn(n_samples, n_features)
    labels = (features[:, 0] + features[:, 1] > 0).astype(int)
    center_ids = (features[:, 2] * 2 + 2).astype(int) % 5
    feature_display_names = [f'Feature {i}' for i in range(n_features)]
    print(f"✅ 生成了 {n_samples} 个模拟样本")

print()

# ========================================
# 2. 计算距离矩阵和Mantel检验
# ========================================
print("=" * 80)
print("📊 步骤 2: 计算距离矩阵和执行Mantel检验")
print("=" * 80)

# 计算特征之间的Pearson相关性
feature_corr = np.corrcoef(features.T)
n_features = len(feature_display_names)

# 计算每个特征对之间的Mantel检验
print("📊 计算特征对之间的Mantel检验...")
mantel_r_matrix = np.zeros((n_features, n_features))
mantel_p_matrix = np.ones((n_features, n_features))

def mantel_test(dist1, dist2, n_permutations=999):
    """Mantel检验"""
    n = dist1.shape[0]
    triu_indices = np.triu_indices(n, k=1)
    vec1 = dist1[triu_indices]
    vec2 = dist2[triu_indices]
    
    if len(vec1) == 0 or np.std(vec1) == 0 or np.std(vec2) == 0:
        return 0.0, 1.0
    
    r_obs, _ = pearsonr(vec1, vec2)
    
    r_perm = []
    for _ in range(n_permutations):
        perm_indices = np.random.permutation(n)
        dist2_perm = dist2[np.ix_(perm_indices, perm_indices)]
        vec2_perm = dist2_perm[triu_indices]
        if np.std(vec2_perm) > 0:
            r_perm_val, _ = pearsonr(vec1, vec2_perm)
            r_perm.append(r_perm_val)
    
    if len(r_perm) > 0:
        p_value = (np.sum(np.array(r_perm) >= r_obs) + 1) / (len(r_perm) + 1)
    else:
        p_value = 1.0
    
    return r_obs, p_value

# 计算特征对之间的Mantel检验
for i in range(n_features):
    for j in range(i+1):
        if i == j:
            mantel_r_matrix[i, j] = 1.0
            mantel_p_matrix[i, j] = 0.0
        else:
            feat_i_dist = pairwise_distances(features[:, i:i+1], metric='euclidean')
            feat_j_dist = pairwise_distances(features[:, j:j+1], metric='euclidean')
            r, p = mantel_test(feat_i_dist, feat_j_dist, n_permutations=99)
            mantel_r_matrix[i, j] = r
            mantel_p_matrix[i, j] = p
            mantel_r_matrix[j, i] = r
            mantel_p_matrix[j, i] = p

print("✅ Mantel检验完成")

# 计算特征与标签/中心的Mantel检验
mantel_results = []
if labels is not None:
    label_dist = pairwise_distances(labels.reshape(-1, 1), metric='hamming') * len(labels)
    for i, feat_name in enumerate(feature_display_names):
        feat_dist = pairwise_distances(features[:, i:i+1], metric='euclidean')
        r, p = mantel_test(feat_dist, label_dist, n_permutations=99)
        mantel_results.append({
            'Feature': feat_name,
            'Target': 'Label',
            'Mantel_r': r,
            'Mantel_p': p
        })

if center_ids is not None:
    center_dist = pairwise_distances(center_ids.reshape(-1, 1), metric='hamming') * len(center_ids)
    for i, feat_name in enumerate(feature_display_names):
        feat_dist = pairwise_distances(features[:, i:i+1], metric='euclidean')
        r, p = mantel_test(feat_dist, center_dist, n_permutations=99)
        mantel_results.append({
            'Feature': feat_name,
            'Target': 'Center',
            'Mantel_r': r,
            'Mantel_p': p
        })

mantel_df = pd.DataFrame(mantel_results)
results_csv = OUTPUT_DIR / 'mantel_test_results_standard.csv'
mantel_df.to_csv(results_csv, index=False)
print(f"✅ 结果已保存: {results_csv}")
print()

# ========================================
# 3. 生成标准样式组合图
# ========================================
print("=" * 80)
print("🎨 步骤 3: 生成标准样式组合图")
print("=" * 80)

# 创建大图
fig = plt.figure(figsize=(18, 10))
fig.patch.set_facecolor(COLORS['background'])
gs = GridSpec(1, 2, figure=fig, width_ratios=[1.2, 1], wspace=0.4, hspace=0.3)

# ========================================
# 左侧：下三角相关性矩阵
# ========================================
print("📊 生成下三角相关性矩阵...")
ax1 = fig.add_subplot(gs[0, 0])

# 创建下三角掩码
mask = np.triu(np.ones_like(feature_corr, dtype=bool), k=1)

# 创建自定义colormap（从红色到白色到蓝色）
from matplotlib.colors import LinearSegmentedColormap
colors_list = ['#8B0000', '#DC143C', '#FF6B6B', '#FFFFFF', '#C7CCD6', '#5B8FA3', '#2874A6']
cmap = LinearSegmentedColormap.from_list('correlation_cmap', colors_list, N=100)

# 创建显示矩阵（下三角 + 对角线）
display_corr = feature_corr.copy()
display_corr[mask] = np.nan

# 创建标注矩阵（包含数值和星号）
annot_matrix = np.full((n_features, n_features), '', dtype=object)

# 在下三角中准备标注
for i in range(n_features):
    for j in range(i+1):
        pearson_r = feature_corr[i, j]
        mantel_p = mantel_p_matrix[i, j]
        
        # 添加显著性星号
        if mantel_p < 0.001:
            sig = '***'
        elif mantel_p < 0.01:
            sig = '**'
        elif mantel_p < 0.05:
            sig = '*'
        else:
            sig = ''
        
        # 组合数值和星号
        if sig:
            annot_matrix[i, j] = f'{pearson_r:.2f}\n{sig}'
        else:
            annot_matrix[i, j] = f'{pearson_r:.2f}'
    
    # 对角线
    annot_matrix[i, i] = '1.00'

# 绘制热图（使用颜色表示Pearson相关性）
im = ax1.imshow(display_corr, cmap=cmap, aspect='auto', vmin=-1, vmax=1, interpolation='nearest')

# 添加数值和星号标注
for i in range(n_features):
    for j in range(i+1):
        pearson_r = feature_corr[i, j]
        mantel_r = abs(mantel_r_matrix[i, j])
        mantel_p = mantel_p_matrix[i, j]
        
        # 方块大小：根据Mantel's r值（通过调整文本位置和添加背景方块实现）
        if mantel_r >= 0.4:
            box_alpha = 0.9
            box_size = 0.7
        elif mantel_r >= 0.2:
            box_alpha = 0.6
            box_size = 0.5
        else:
            box_alpha = 0.3
            box_size = 0.3
        
        # 获取颜色
        norm_val = (pearson_r + 1) / 2  # 归一化到0-1
        box_color = cmap(norm_val)
        
        # 绘制背景方块（表示Mantel's r值大小）
        x_pos = j
        y_pos = i
        rect = Rectangle((x_pos - box_size/2, y_pos - box_size/2),
                        box_size, box_size,
                        facecolor=box_color, alpha=box_alpha,
                        edgecolor='white', linewidth=0.5,
                        transform=ax1.transData)
        ax1.add_patch(rect)
        
        # 添加数值和星号
        text = annot_matrix[i, j]
        lines = text.split('\n')
        ax1.text(j, i, lines[0], ha='center', va='center',
                fontsize=8, fontweight='bold',
                color='white' if abs(pearson_r) > 0.5 else 'black',
                transform=ax1.transData)
        
        if len(lines) > 1 and lines[1]:
            # 星号颜色
            if mantel_p < 0.001:
                sig_color = '#8B0000'
            elif mantel_p < 0.01:
                sig_color = COLORS['positive']
            elif mantel_p < 0.05:
                sig_color = COLORS['center_2']
            else:
                sig_color = '#CCCCCC'
            
            ax1.text(j, i + 0.3, lines[1], ha='center', va='bottom',
                    fontsize=9, fontweight='bold', color=sig_color,
                    transform=ax1.transData)

# 设置刻度
ax1.set_xticks(range(n_features))
ax1.set_yticks(range(n_features))
ax1.set_xticklabels(feature_display_names, rotation=45, ha='right', fontsize=8)
ax1.set_yticklabels(feature_display_names, rotation=0, fontsize=8)

# 添加colorbar
cbar = plt.colorbar(im, ax=ax1, shrink=0.6, pad=0.02)
cbar.set_label("Pearson's correlation", fontsize=10, fontweight='bold')

ax1.set_title("Pearson's Correlation Matrix\n(with Mantel Test)", 
             fontsize=13, fontweight='bold', pad=12)
ax1.set_facecolor(COLORS['background'])
plt.xticks(rotation=45, ha='right', fontsize=8)
plt.yticks(rotation=0, fontsize=8)

print("✅ 相关性矩阵已生成")

# ========================================
# 右侧：网络图
# ========================================
print("📊 生成网络图...")
ax2 = fig.add_subplot(gs[0, 1])
ax2.set_facecolor(COLORS['background'])
ax2.axis('off')

# 创建网络图
G = nx.Graph()

# 添加特征节点
for node in feature_display_names:
    G.add_node(node, node_type='feature')

# 添加目标节点
target_nodes = []
if label_dist is not None:
    G.add_node('Label', node_type='target')
    target_nodes.append('Label')
if center_dist is not None:
    G.add_node('Center', node_type='target')
    target_nodes.append('Center')

# 添加边
for _, row in mantel_df.iterrows():
    feature = row['Feature']
    target = row['Target']
    r = row['Mantel_r']
    p = row['Mantel_p']
    
    # 显示所有连接，但根据显著性调整透明度
    G.add_edge(feature, target, weight=abs(r), p_value=p, r_value=r)

# 设置节点位置
pos = {}

# 特征节点：左侧排列
feature_y_positions = np.linspace(1.2, -1.2, len(feature_display_names))
for i, node in enumerate(feature_display_names):
    pos[node] = (-1.5, feature_y_positions[i])

# 目标节点：右侧排列
target_y_positions = np.linspace(0.6, -0.6, len(target_nodes))
for i, node in enumerate(target_nodes):
    pos[node] = (1.5, target_y_positions[i])

# 绘制边
edges = G.edges(data=True)
for u, v, data in edges:
    r = data['r_value']
    p = data['p_value']
    weight = abs(data['weight'])
    
    # 线条粗细：根据Mantel's r值
    if weight >= 0.4:
        width = 4.0
    elif weight >= 0.2:
        width = 2.5
    else:
        width = 1.0
    
    # 线条颜色：根据p值
    if p < 0.001:
        color = '#8B0000'  # 深红色
    elif p < 0.01:
        color = COLORS['positive']  # 粉棕色
    elif p < 0.05:
        color = COLORS['center_2']  # 中粉棕灰
    else:
        color = '#CCCCCC'  # 灰色
    
    nx.draw_networkx_edges(G, pos, edgelist=[(u, v)], width=width,
                          edge_color=color, alpha=0.8, ax=ax2, style='solid')

# 绘制节点
feature_nodes_list = [n for n in G.nodes() if G.nodes[n]['node_type'] == 'feature']
target_nodes_list = [n for n in G.nodes() if G.nodes[n]['node_type'] == 'target']

nx.draw_networkx_nodes(G, pos, nodelist=feature_nodes_list,
                      node_color=COLORS['center_2'], node_size=2000,
                      alpha=0.9, ax=ax2, node_shape='o')
nx.draw_networkx_nodes(G, pos, nodelist=target_nodes_list,
                      node_color=COLORS['positive'], node_size=3000,
                      alpha=0.9, ax=ax2, node_shape='s')

# 绘制标签
nx.draw_networkx_labels(G, pos, font_size=9, font_weight='bold', ax=ax2)

ax2.set_xlim([-2, 2])
ax2.set_ylim([-1.5, 1.5])
ax2.set_title("Mantel Test Network\n(Features vs Targets)", 
             fontsize=13, fontweight='bold', pad=12)

# 添加图例
from matplotlib.patches import Patch
from matplotlib.lines import Line2D

# 图例1：Mantel's p value（颜色）
legend_elements_p = [
    Patch(facecolor='#8B0000', label='p < 0.001'),
    Patch(facecolor=COLORS['positive'], label='0.001 ≤ p < 0.01'),
    Patch(facecolor=COLORS['center_2'], label='0.01 ≤ p < 0.05'),
    Patch(facecolor='#CCCCCC', label='p ≥ 0.05'),
]

legend1 = ax2.legend(handles=legend_elements_p, loc='upper left',
                    title="Mantel's p value", fontsize=8, title_fontsize=9,
                    framealpha=0.95, edgecolor='black', frameon=True)
legend1.get_frame().set_facecolor('white')

# 图例2：Mantel's r value（线条粗细）
legend_elements_r = [
    Line2D([0], [0], color='black', lw=4, label='r ≥ 0.4'),
    Line2D([0], [0], color='black', lw=2.5, label='0.2 ≤ r < 0.4'),
    Line2D([0], [0], color='black', lw=1, label='r < 0.2'),
]

legend2 = ax2.legend(handles=legend_elements_r, loc='lower left',
                    title="Mantel's r value", fontsize=8, title_fontsize=9,
                    framealpha=0.95, edgecolor='black', frameon=True)
legend2.get_frame().set_facecolor('white')

# 恢复第一个图例
ax2.add_artist(legend1)

print("✅ 网络图已生成")

# 保存
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'Mantel_Test_Standard.png', dpi=300, bbox_inches='tight',
           facecolor=COLORS['background'])
plt.savefig(FIGURES_DIR / 'Mantel_Test_Standard.pdf', dpi=300, bbox_inches='tight',
           facecolor=COLORS['background'])
plt.close()
print("✅ 标准样式组合图已保存")

print()

# ========================================
# 完成
# ========================================
print("=" * 80)
print("✅ Mantel检验分析完成！")
print("=" * 80)
print(f"📁 输出目录: {OUTPUT_DIR}")
print(f"📊 结果CSV: {results_csv}")
print(f"🎨 图表目录: {FIGURES_DIR}")
print()
print("生成的文件:")
print(f"  ✅ mantel_test_results_standard.csv")
for fig_file in sorted(FIGURES_DIR.glob('Mantel_Test_Standard.*')):
    size = fig_file.stat().st_size / 1024
    print(f"  ✅ {fig_file.name} ({size:.1f} KB)")
print("=" * 80)

